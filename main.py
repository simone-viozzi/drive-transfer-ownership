...
class Drive:
    ...
    def list_files(self, path: str, folder_id='root', index=0, debug=False) -> "list[GoogleDriveFile]":
        ...
        if path in self.files_cache:
            printd(f"path in cache", debug)
            return self.files_cache[path]
        printd(f"path not in cache", debug)

        folders = path.split('/')[:-1 or None]

        l: "list[GoogleDriveFile]" = self.drive.ListFile({'q': f"'{folder_id}' in parents and trashed=false"}).GetList()

        cache_path = '/'
        for folder in folders[1:index+1]:
            cache_path += f'{folder}/'

        titles = map(lambda f: f['title'], l)
           
        printd(f"cache_path {cache_path} -> {list(titles)}", debug)

        self.files_cache[cache_path] = l
...
if __name__ == '__main__':
    ...
    l = univpm1.list_files("/tmp_folder/bla")
    ...
    print("the cache is:")
    print(univpm1.files_cache.keys())

    print("\n" + "*" * 80)
    print(f"listing files in /tmp_folder/")

    l = univpm1.list_files("/tmp_folder/")
